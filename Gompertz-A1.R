...
plot(time, mass, xlab = "time (years)", ylab = "absorbance readings", main = "Gompertz model",
     cex.axis = 1.5, cex.lab = 1.5, pch =19) 
curve(fitgompertz, add=TRUE, lwd = 2, col = "blue")

#calculate BIC

bic <- 2*loglikG(par = OPTG$par)+2*log(109)
...
#bic
bic_bayesian <- 2*loglikG(par = mean)+2*log(109) #bic
